...
2. On a PC Run `chartpress --push`. On a Mac run `chartpress --push --builder docker-buildx --platform linux/amd64`.  
   This will build and push the image, *but not put anything in YAML*. 
   There is no place we can put this in `values.yaml`, since this is only used for data100.
